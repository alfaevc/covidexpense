...
According to the heat map, we can easily tell that many of the CPI items are highly and positively correlated with each other. However, `Public Transportation`, `Medical Care Services`, `Communication`, and `Apparel` showed white and red color in the heat map. This indicates that they are not or negatively correlated with other items. Combining with the trend graph, I know that their prices did not increase much. It is also reasonable because some of them have pricing mechanism that is not determined by the market. For instance, `Public Transportation`, `Medical Care Services` and `Communication` 
...
state.centers.df = map.df %>% group_by(state) %>% summarise(x.mid = mid(x), y.mid = mid(y), group=mean(group))
map.state.names = geom_text(data=state.centers.df, aes(x=x, y=y.mid, group=group, label=state),size=1.5, hjust=0, vjust=0, colour = 1)
map.state.points = geom_point(data=state.centers.df, aes(x=x.mid, y=y.mid, group=group), size = 0.5, alpha = 0.6, colour = 1)
```
