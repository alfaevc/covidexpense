...
cdc_all = read.csv("data/cdc_all.csv")
cdc_monthly_all_state = read.csv("data/cdc_monthly_all_state.csv")
cpi_monthly = read.csv("data/cpi_monthly.csv")
data_monthly = read.csv("data/data_monthly.csv")
...
>>>>>>> 7d7820038be7eb7c9436801c392be567e6c2b744
```{r, message=FALSE, warning=FALSE, echo=FALSE}
cpi.sub <- cpi %>% 
...
### new cases and total cases by State from 2020 Jan to 2021 Nov
We compared how new cases changed by states from 2020 Jan to 2021 Nov 
...
cdc_monthly_all_state = cdc %>% 
  select(month, new_death, new_case, conf_cases, conf_death) %>% 
  group_by(month) %>% 
  summarise(new_death=sum(new_death),new_case=sum(new_case), tot_cases=sum(conf_cases), tot_death=sum(conf_death))

cdc_monthly_all_state %>% pivot_longer(cols = new_death:tot_death, names_to='var',values_to='value') %>%
  ggplot( aes(x=month, y=value, group=var, color=var)) +
    geom_line() + ggtitle("Covid-19 Data Trend") + theme(axis.text.x = element_text(angle = 90), plot.title = element_text(size = 40, face = "bold"))
```
...
The above motion map shows that the first five cases in the United States are in the Illinois, Arizona and Washington state. 
```{r}

cdcmonthdeath = cdc.month %>% group_by(statefull)%>%arrange(date)%>% 
    do(item = list(
        state = first(.$statefull),
        sequence = .$new_death,
...
cor_matrix = inner_join(cpi_monthly, cdc_monthly_all_state, by=c("date"="month")) %>% select(-date, -new_case, -new_death) %>% cor() %>% as.data.frame()
