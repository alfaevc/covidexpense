...
cdc_all = read_csv("data/cdc_all.csv")
cdc_monthly_all_state = read_csv("data/cdc_monthly_all_state.csv")
cpi_monthly = read_csv("data/cpi_monthly.csv")
data_monthly = read_csv("data/data_monthly.csv")
cdc_all = cdc_all %>% mutate(date=as.Date(date), month=as.yearmon(format(as.Date(date), "%Y-%m")))
cdc_monthly_all_state = cdc_monthly_all_state %>% mutate(month=as.yearmon(month), "%Y-%m")
...
From the above motion map, we can see that the first five cases in the United States come from Illinois State, Washington State and Arizona State. At the beginning, California and New York state has the most new cases which are two of the largest States that have the most population. From Jun 2020 to Sep 2020, California, Texas and Florida States ahve the most new cases.
```{r}
