...
DF <- data.frame(description = character(),
                 ...
                 avgvalue=numeric(),
                 date=character()) 
temp = data %>% select(date,avgvalue) %>% filter(description=="Northeast")
for(e in Northeast){
  temp$state = e
  DF = bind_rows(DF,temp)
} 
temp = data %>% select(date,avgvalue) %>% filter(description=="Mid.Atlantic")
for(e in Mid.Atlantic){
  temp$state = e
  DF = bind_rows(DF,temp)
} 
temp = data %>% select(date,avgvalue) %>% filter(description=="Southeast")
for(e in Southeast){
  temp$state = e
  DF = bind_rows(DF,temp)
} 
temp = data %>% select(date,avgvalue) %>% filter(description=="Midwest")
for(e in Midwest){
  temp$state = e
  DF = bind_rows(DF,temp)
} 
temp = data %>% select(date,avgvalue) %>% filter(description=="Southwest")
for(e in Southwest){
  temp$state = e
  DF = bind_rows(DF,temp)
} 
temp = data %>% select(date,avgvalue) %>% filter(description=="West")
for(e in West){
  temp$state = e
  DF = bind_rows(DF,temp)
} 
...
highchart(type = "map") %>% 
  ...
  hc_title(text = "CPI change from 2019 Jan to 2021 Nov by region") %>%
  ...
  hc_legend(title = list(text = "cpi"), valueDecimals = 0, valueSuffix = "%") %>%
