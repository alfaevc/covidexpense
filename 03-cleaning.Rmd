...
Observe that covid data are daily based and cpi data are monthly based, we use ```dplyr``` to shrink covid data to monthly based by summing the number of cases and deaths for every state, where we denote this new dataset ```cdcmonth.csv``` after adjusting date into the ```yearmon``` format. We also noticed that for the attributes ```consent_cases``` and```consent_deaths``` some ```NA```s are denoted as the string ```'N\A'```, where it makes sense to replace them with actual ```NA``` since they are indeed missing values. 
