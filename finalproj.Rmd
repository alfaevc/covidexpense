...
cpi %>%
  ggplot( aes(x=date, y=value, group=item_name, color=item_name)) +
    geom_line() + ggtitle("CPI Time Series for All Items") + theme(axis.text.x = element_text(angle = 90), plot.title = element_text(size = 40, face = "bold"))
```
...
cdc$month <- as.yearmon(cdc$month, "%Y-%m")
...
#cor_matrix %>% select("All items", )
#cor_matrix = inner_join(cpi_monthly, cdc_monthly_all_state, by=c("date"="month")) %>% 
#  select("tot_death", "tot_cases", "Apparel", "Communication" , "Public transportation", "Education") %>% 
#  cor()
#hchart.cor(cor_matrix)
...
<!--chapter:end:cpibyregion.Rmd-->




<!--chapter:end:cpidata.Rmd-->




<!--chapter:end:regressions.Rmd-->
